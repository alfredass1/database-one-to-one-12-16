...
def insert_record(customer, transaction):
    ...
    customer_id_for_transaction = cursor.execute(
        "SELECT customer_id FROM customers WHERE customer_name = 'Alfredas'").fetchone()
    connection.close()
    ...
    transaction_id_for_customer = cursor.execute(
        "SELECT transaction_id FROM transactions ORDER BY transaction_id DESC LIMIT 1").fetchone()
...
customer1 = customer(None, "Alfredas", "Gilvickas", None)
